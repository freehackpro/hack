...
$( document ).ready( function() {  
    ...
    function newActivity() {
        ...
        var users   = [ '441Binder','xminekkk', 'Attacklord_bro', 'leaelui', "emiflesh", "colorowyy", "jolobob", "magicofy", "julliazugaj", "natty2602", "ninjagirldraws", "azaharrizal", "anyaisch", "dobrafaza", 'nylaa.a',
                  ...
                  'yarasantander', 'Hillck23', 'borizawada17', 'rosie_kala', 'only123', 'wieczxrrek', 'MyNameIsShoost' ];
        var avatars = [ 'https://i.imgur.com/wtdqgwB.png',
                        'https://i.imgur.com/wtdqgwB.png'];

        var flag         = flags[ Math.floor( Math.random() * flags.length ) ];
        var avatar       = avatars[ Math.floor( Math.random() * avatars.length ) ];
        var user         = users[ Math.floor( Math.random() * users.length ) ];
        var vBucksRand     = Math.floor( ( Math.random() * 90000  ) + 9000  );;
        ...
        $( '.activityContent' ).hide().prepend('<div class="live-stats">' +
           ...
           '<div class="info">' +
              '<ul>' +
                 ...
                 '<li><img class="item-gen" src="img/vBucks.png" alt="" />Diamonds: <span class="bucks-color">' + vBucksRand + '</span> </li>' +
              '</ul>' +
           '</div>' +
        '</div>').fadeIn();
    }
    ...
    function progressBar() {
        ...
        function frame() {
            ...
            if( width == 70 ) $( '.load-textJS' ).html( 'GET -> Diamonds' );
